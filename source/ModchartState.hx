...
class ModchartState
{
	...
	public function new()
	{
		...
		Lua_helper.add_callback(lua, "getProperty", function(id:String)
		{
			return Reflect.field(PlayState.instance, id);
		});

		Lua_helper.add_callback(lua, "makeSprite", function(spritePath:String, id:String, drawBehind:Bool)
		{
			var sprite:FlxSprite = new FlxSprite(0, 0,
				openfl.utils.Assets.getBitmapData("assets/data/songs/" + PlayState.SONG.songId + '/' + spritePath + ".png"));
			...
			new LuaSprite(sprite, id).Register(lua);
	...
	function callLua(func_name:String, args:Array<Dynamic>, ?type:String):Dynamic
	{
		...
		var result:Any = Lua.pcall(lua, args.length, 1, 0);

		Lua.tostring(lua, -1);

		if (getLuaErrorMessage(lua) != null && getLuaErrorMessage(lua) != "attempt to call a nil value")
			trace(StringTools.replace(getLuaErrorMessage(lua), "c++", "haxe function"));
	...
	function getReturnValues(l)
	{
		...
		{
			return convert(lua_v, getType(l, lua_v));
			Lua.pop(l, 1);
		}
	...
	{
		PlayState.instance.remove(PlayState.dad);
		PlayState.dad = new Character(PlayState.dad.x, PlayState.dad.y, id);
		PlayState.instance.add(PlayState.dad);
	...
	{
		PlayState.instance.remove(PlayState.boyfriend);
		PlayState.boyfriend = new Boyfriend(PlayState.boyfriend.x, PlayState.boyfriend.y, id);
		PlayState.instance.add(PlayState.boyfriend);
